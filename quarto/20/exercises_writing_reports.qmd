---
title: "Exercises: Writing Scientific Reports in LaTeX"
---
...
Comprehensive guidance on using Overleaf can be found in the [Overleaf documentation](https://docs.overleaf.com/getting-started/how-do-i-use-overleaf).

An integrated LaTeX-Overleaf tutorial is available at [Overleaf Learn LaTeX in 30 minutes](https://www.overleaf.com/learn/latex/Learn_LaTeX_in_30_minutes).
...
**Objective:** Familiarize yourself with the [specific template provided for your Project 4 report (download)](report.zip) and understand how it differs from the standard `article` class.
...
When including the figure you will use the `\includegraphics{}` command, which allows you to specify the width of the figure (e.g. `width=0.8\textwidth` for 80% of the text width). For example:

```latex
\begin{figure}[t]
    \centering
    \includegraphics[width=0.8\columnwidth]{my_figure.png}%this reduces the width of the figure
    \caption{This is the caption for the figure.}
    \label{fig:my_figure}
\end{figure}
```

In general, it is reccomended to set figure fontsizes around 10-11 points and to fix the size of the figure, so that you can include it directly without distorting/resizing it. In this case, the inclusion would be simpler.


```latex
\begin{figure}[t]
    \centering
    \includegraphics {my_figure.png}%no need to specify width if the figure is already sized appropriately
    \caption{This is the caption for the figure.}
    \label{fig:my_figure}
\end{figure}
```
...
If the figure is mostly lines, you may want to use a vector format like PDF or SVG. If it is a photo or has many color (e.g. a colormap), a raster format like PNG is fine.

Once your figure is ready, upload your figure to your Overleaf project (e.g. via drag and drop).

Place the figure in the *Results* section of your document  using `figure` with the **`[t]` or `[tb]` placement specifier**. Inside the environment, use **`\includegraphics{}`** to **insert your image**.  Play with other placement options like `[h]` (here) or `[b]` (bottom) to see how LaTeX handles figure placement.

Add a **caption** beneath the figure using `\caption{}` that **describes what the figure shows** in a clear and informative way. Use **`\label{fig:something}`** to **give your figure a meaningful label**. Then, in the text above or below the figure, **reference it** using `\ref{fig:something}`.
...
In your LaTeX document, include `\usepackage{natbib}` or rely on the default bibliography handling. Near the end of your document (after your Conclusions section), add **`\bibliographystyle{}` with a suitable style (see the correspoding section in the [lecture](./lecture_writing_reports.qmd)).
